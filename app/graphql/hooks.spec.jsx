...
describe("makeUseQuery", () => {
    ...
    it("should return eventually the result of the executeQuery callback", async () => {
        const { getByTestId } = render(<TestComponent variables={{ cubeIri: "example1" }}/>);
        expect(getByTestId("result").innerHTML).toEqual("loading");
        await waitFor(() => expect(getByTestId("result").innerHTML).toEqual("mock data"));
    });
    it("should not re-execute the query if data is available", async () => {
        const { getByTestId, rerender } = render(<TestComponent variables={{ cubeIri: "example1" }}/>);
        await waitFor(() => expect(getByTestId("result").innerHTML).toEqual("mock data"));
...
describe("useComponentsQuery - keepPreviousData", () => {
    ...
    beforeEach(() => {
        ...
        global.fetch = async (_url, options) => {
            ...
            const isGebaudeCube = reqBody.variables.cubeFilter.iri.includes("https://energy.ld.admin.ch/sfoe/bfe_ogd18_gebaeudeprogramm_energiewirkung/6");
            await sleep(1);
    ...
    it("should work when keep previous data is false", async () => {
        await act(async () => {
            ...
            const dimensions = (_a = hook.result.current[0].data) === null || _a === void 0 ? void 0 : _a.dataCubesComponents.dimensions;
...
describe("useDataCubesComponentsQuery", () => {
    describe("transformDataCubesComponents", () => {
        const measure1Id = "measure1";
        const measure2Id = "measure2";
        const mockData = {
            data: {
                dataCubesComponents: {
                    dimensions: [],
        ...
        it("should transform measure values and limits with conversion units", () => {
            ...
            const result = transformDataCubesComponents(mockData, {
                ...
                conversionUnitsByComponentId: mockOptions.chartConfig.conversionUnitsByComponentId,
            });
            expect((_a = result.data) === null || _a === void 0 ? void 0 : _a.dataCubesComponents.measures[0]).toEqual({
                id: measure1Id,
                label: "Measure 1",
                unit: "lbs",
                originalUnit: "kg",
            ...
            expect((_b = result.data) === null || _b === void 0 ? void 0 : _b.dataCubesComponents.measures[1]).toEqual(mockData.data.dataCubesComponents.measures[1]);
        });
        it("should return unchanged data when no conversion units are provided", () => {
            ...
            const result = transformDataCubesComponents(mockData, optionsWithoutConversion);
            expect(result).toEqual(mockData);
        });
        it("should return unchanged data when data is null", () => {
            const result = transformDataCubesComponents({ data: null, fetching: false }, {
                locale: mockOptions.variables.locale,
                conversionUnitsByComponentId: mockOptions.chartConfig.conversionUnitsByComponentId,
            });
        ...
        it("should return unchanged data when data is undefined", () => {
            const result = transformDataCubesComponents({ data: undefined, fetching: false }, {
                locale: mockOptions.variables.locale,
                conversionUnitsByComponentId: mockOptions.chartConfig.conversionUnitsByComponentId,
            });
...
describe("useDataCubesObservationsQuery", () => {
    describe("transformDataCubesObservations", () => {
        ...
        it("should transform observation values with conversion units", () => {
            ...
            expect((_a = result.data) === null || _a === void 0 ? void 0 : _a.dataCubesObservations.data).toEqual([
                {
                    [measure1Id]: 200,
                    [measure2Id]: 600,
                    measure3: 300,
        ...
        it("should return unchanged data when no conversion units are provided", () => {
            ...
            const result = transformDataCubesObservations(mockData, optionsWithoutConversion);
            expect(result).toEqual(mockData);
        });
        it("should return unchanged data when data is null", () => {
            const result = transformDataCubesObservations({ data: null, fetching: false }, {
                locale: mockOptions.variables.locale,
                conversionUnitsByComponentId: mockOptions.chartConfig.conversionUnitsByComponentId,
            });
            expect(result).toEqual({ data: null, fetching: false });
        ...
        it("should return unchanged data when data is undefined", () => {
            const result = transformDataCubesObservations({ data: undefined, fetching: false }, {
                locale: mockOptions.variables.locale,
                conversionUnitsByComponentId: mockOptions.chartConfig.conversionUnitsByComponentId,
            });
            expect(result).toEqual({ data: undefined, fetching: false });
