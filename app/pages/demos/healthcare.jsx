...
export default function HealthcareDemo() {
    ...
    const longestWait = waitingLists.reduce((max, item) => item.averageWaitDays > max.averageWaitDays ? item : max);
    return (<DemoLayout title={title} description={description} datasetInfo={{
      ...
      <Box>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            return (<Box key={index} sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ position: 'relative' }}>
                    <LinearProgress variant="determinate" value={Math.min((exceedanceRatio / 4) * 100, 100)} sx={{
                    height: 28,
                    borderRadius: 1,
        ...
        <Paper elevation={2} sx={{ p: 3, mb: 4 }}>
          ...
          <LineChart data={healthcareWorkerExodus.map(item => ({
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <Box sx={{ mb: 2 }}>
                ...
                <LineChart data={hospitalCapacity.map(item => ({
            label: item.year.toString(),
        ...
        <Paper elevation={2} sx={{ p: 3 }}>
          ...
          <Grid container spacing={2}>
            {healthIndicators.map((indicator, index) => (<Grid item xs={12} md={6} key={index}>
                <Card variant="outlined">
                  <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                      ...
                      <Chip label={indicator.trend === 'worsening'
                ? (locale === 'sr' ? 'Pogoršanje' : 'Worsening')
                : indicator.trend === 'improving'
                    ? (locale === 'sr' ? 'Poboljšanje' : 'Improving')
                    : (locale === 'sr' ? 'Stabilno' : 'Stable')} size="small" color={indicator.trend === 'worsening'
                ? 'error'
                : indicator.trend === 'improving'
                    ? 'success'
                    : 'default'}/>
