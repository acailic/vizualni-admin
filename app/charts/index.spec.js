...
describe("initial config", () => {
    it("should create an initial table config with column order based on dimension order", () => {
        const config = getInitialConfig({
            ...
            dimensions: forestAreaData.data.dataCubeByIri
                .dimensions,
            measures: forestAreaData.data.dataCubeByIri.measures,
        });
        expect(Object.values(config.fields).map((x) => [
            x["componentId"],
            x["index"],
        ])).toEqual([
    ...
    it("should create an initial column config having x axis correctly inferred (temporal ordinal)", () => {
        ...
        expect(config.fields.x.componentId).toEqual(stringifyComponentId({
            unversionedCubeIri: "https://cube-iri",
            unversionedComponentIri: "temporal-ordinal-dimension-iri",
        }));
    });
    ...
    it("should create an initial scatterplot config having segment correctly defined", () => {
        ...
        expect(config.fields.palette).toBeUndefined();
    });
});
describe("enabled chart types", () => {
    ...
    it("should only allow table if there are only measures available", () => {
        ...
        expect(Object.entries(possibleChartTypesDict)
            .filter(([k]) => k !== "table")
            .every(([, { message }]) => !!message)).toBe(true);
    });
...
describe("chart type switch", () => {
    it("should correctly remove non-allowed interactive data filters", () => {
        const chartConfig = {
            ...
            fields: {
                x: {
                    componentId: "https://environment.ld.admin.ch/foen/ubd0104/parametertype",
                },
                y: {
        ...
        const newConfig = getChartConfigAdjustedToChartType({
            ...
            dimensions: bathingWaterData.data.dataCubeByIri
                .dimensions,
            measures: bathingWaterData.data.dataCubeByIri.measures,
        });
        expect(newConfig.interactiveFiltersConfig.dataFilters.active).toEqual(false);
    ...
    it("should not carry over not-allowed segment", () => {
        const oldChartConfig = {
            ...
            cubes: [
                {
                    ...
                    filters: {
                        ...
                        "https://environment.ld.admin.ch/foen/ubd000502/sektorid": {
                            type: "single",
                            value: "https://environment.ld.admin.ch/vocabulary/ghg_emission_sectors_co2_ordinance/3",
                        },
                    },
                },
            ],
            ...
            fields: {
                ...
                y: {
                    componentId: "https://environment.ld.admin.ch/foen/ubd000502/werteNichtGerundet",
                },
                color: {
                    type: "segment",
                    paletteId: "category10",
        ...
        expect(newChartConfig.fields.x.componentId).not.toEqual(oldChartConfig.fields.x.componentId);
    });
    it("should carry over colors", () => {
        ...
        expect(newChartConfig.fields.color.paletteId).toEqual(oldChartConfig.fields.color.paletteId);
        expect(newChartConfig.fields.color.type).toEqual("segment");
        expect(newChartConfig.fields.color.colorMapping["A"]).toEqual(oldChartConfig.fields.color.colorMapping["A"]);
    });
    it("should prefer to use components from different cubes if merging cubes, otherwise not", () => {
        ...
        expect(newChartConfigMergingOfCubes.fields.y.rightAxisComponentId.startsWith("A")).toEqual(true);
        expect(newChartConfigMergingOfCubes.fields.y.leftAxisComponentId.startsWith("B")).toEqual(true);
        ...
        expect(newChartConfigNotMergingOfCubes.fields.y.rightAxisComponentId.startsWith("A")).toEqual(true);
        expect(newChartConfigNotMergingOfCubes.fields.y.leftAxisComponentId.startsWith("A")).toEqual(true);
    });
});
